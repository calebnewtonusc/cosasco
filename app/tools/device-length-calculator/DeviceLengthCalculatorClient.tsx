...
type CalcAction =
  | { type: 'SET_PIPE_IDX'; idx: number; schedule: ScheduleKey }
  | { type: 'SET_SCHEDULE'; schedule: ScheduleKey }
  | { type: 'SET_FITTING'; fitting: string }
  | { type: 'SET_POSITION'; position: string }
  | { type: 'SET_CUSTOM_DEPTH'; depth: string }
...
function calcReducer(state: CalcState, action: CalcAction): CalcState {
  switch (action.type) {
    case 'SET_PIPE_IDX':
      return { ...state, pipeIdx: action.idx, schedule: action.schedule }
    case 'SET_SCHEDULE':
      return { ...state, schedule: action.schedule }
    case 'SET_FITTING':
      return { ...state, fitting: action.fitting }
    case 'SET_POSITION':
      return { ...state, position: action.position }
    case 'SET_CUSTOM_DEPTH':
      return { ...state, customDepth: action.depth }
    case 'SET_UNITS':
...
export default function DeviceLengthCalculatorClient() {
  const [state, dispatch] = useReducer(calcReducer, initialState)
  const { pipeIdx, schedule, fitting, position, customDepth, units } = state
                  ...
                  {PIPE_SIZES.map((p, idx) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        const newSchedule = schKeys.includes(schedule) ? schedule : (schKeys.find(s => s === 'Sch 40') ?? schKeys[0])
                        dispatch({ type: 'SET_PIPE_IDX', idx, schedule: newSchedule })
                      }}
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                  {availableSchedules.map((sch) => (
                    <button
                      key={sch}
                      type="button"
                      onClick={() => dispatch({ type: 'SET_SCHEDULE', schedule: sch })}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {Object.keys(FITTING_ENGAGEMENT).map((f) => (
                    <button
                      key={f}
                      type="button"
                      onClick={() => dispatch({ type: 'SET_FITTING', fitting: f })}
                  ...
                  {POSITIONS.map((pos) => (
                    ...
                    >
                      <input
                        ...
                        onChange={() => dispatch({ type: 'SET_POSITION', position: pos.value })}
                  ...
                  <div className="mt-3">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        step="0.25"
                        value={customDepth}
                        onChange={(e) => dispatch({ type: 'SET_CUSTOM_DEPTH', depth: e.target.value })}
