...
function supportReducer(state: SupportState, action: SupportAction): SupportState {
  switch (action.type) {
    ...
    case 'ADD_FILES': {
      ...
      const next = action.files.filter(f => !existing.has(f.name))
      return { ...state, files: [...state.files, ...next] }
    }
    case 'REMOVE_FILE':
...
export default function SupportPageClient() {
  ...
  const { form, files, status, errorMsg, dragging } = state
  ...
  function addFiles(incoming: FileList | null) {
    if (!incoming) return
    dispatch({ type: 'ADD_FILES', files: Array.from(incoming) })
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      dispatch({ type: 'SET_ERROR', message: 'Something went wrong. Please try again or call us at +1 (562) 949-0123.' })
    }
  }
                      ...
                      <ul className="mt-3 space-y-1.5">
                        {files.map((f) => (
                          <li key={f.name} className="flex items-center justify-between bg-[#f7f9fc] border border-[#e8edf2] rounded-lg px-3 py-2 text-xs">
                            ...
                            <button
                              type="button"
                              onClick={() => dispatch({ type: 'REMOVE_FILE', name: f.name })}
